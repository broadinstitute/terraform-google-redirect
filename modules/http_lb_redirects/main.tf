locals {
  all_hosts = merge([for r in values(var.redirects) : r.certificates]...)
}
...
resource "google_certificate_manager_certificate_map" "redirects" {
  description = "Certificate map for redirects"
...
module "certificates" {
  ...
  certificate_map        = google_certificate_manager_certificate_map.redirects
  certificates           = local.all_hosts
...
resource "google_compute_url_map" "https_url_map" {
  ...
  dynamic "host_rule" {
    ...
    iterator = host
    content {
      hosts        = flatten([for name, certs in host.value.certificates : certs])
  ...
  dynamic "path_matcher" {
    ...
    content {
      ...
      default_url_redirect {
        host_redirect          = site.value.default_destination_host
        path_redirect          = site.value.default_destination_path
        redirect_response_code = site.value.default_redirect_response_code
        strip_query            = false
      }

      dynamic "path_rule" {
        for_each = site.value.redirects
        iterator = rule
